...
interface ProfileEditProfileEditBannerPopupProps {
  isOpen: boolean;
  imageFile: File;
  onClose: () => void;
...
const ProfileEditProfileEditBannerPopup: React.FC<
  ProfileEditProfileEditBannerPopupProps
> = ({ isOpen, imageFile, onClose, onSave }) => {
  ...
  // Generate cropped banner image using canvas
  const getCroppedImg = async (
    ...
    return new Promise((resolve, reject) => {
      canvas.toBlob((blob) => {
        if (blob) {
          const croppedFile = new File([blob], "cropped-banner.jpeg", {
            type: "image/jpeg",
          });
          resolve(croppedFile);
        } else {
...
export default ProfileEditProfileEditBannerPopup;
